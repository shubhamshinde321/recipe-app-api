...
class RecipeImageUploadTests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = get_user_model().objects.create(
            email='shubham.shinde32@gmail.com',
            password='shubham',
        )
    ...
    def test_filter_recipe_by_tags(self):
        ...
        recipe2 = sample_recipe(user=self.user, title='Dal Makhani')
        tag1 = sample_tag(user=self.user, name='Thai')
        tag2 = sample_tag(user=self.user, name='Punjabi')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = sample_recipe(user=self.user, title='kolhapuri chicken')
    ...
    def test_filter_recipe_by_ingredients(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='Puran Poli')
        recipe2 = sample_recipe(user=self.user, title='Kheer')
        ingredient1 = sample_ingredient(user=self.user, name='Jaggery')
        ingredient2 = sample_ingredient(user=self.user, name='almonds')
        recipe1.ingredients.add(ingredient1)
        recipe2.ingredients.add(ingredient2)
        recipe3 = sample_recipe(user=self.user, title='Chicken Rice')
